...
SUPPORTED_CODEBOOKS = ("mcg", "mul1")
CODEBOOK_MARKER = {
    "mcg": MCG_MARKER_SIGNED_INT32,
    "mul1": MUL1_MARKER_SIGNED_INT32,
}
EXL3_BASE_SUFFIXES = ("trellis", "suh", "svh")
# Both markers are accepted when parsing a checkpoint name; a given artifact
# ships exactly one of them and the config's `codebook` decides which.
EXL3_SUFFIXES = EXL3_BASE_SUFFIXES + SUPPORTED_CODEBOOKS
RANK_STACKED_TP = 4
RANK_STACKED_INTERMEDIATE = 512
_RANK_IN_NAME = re.compile(r"(?:^|[._])rank(?P<rank>\d+)(?=[._])")
SWIGLU_LIMIT_DEFAULT = 10.0
# E5: fused-kernel per-expert row capacity. exllamav3_ext.exl3_moe derives
# max_tokens_per_expert from temp_state_g.size(1); experts with more routed
# rows than this are skipped by the fused kernel and fall back to the
# host-syncing Python loop. 128 forces that fallback for nearly every expert
# during prefill. Env-tunable, clamped; allocation happens once at load
# (before CUDA graph capture). Receipt: EXPERIMENTS 2026-09-18 E5.
TEMP_ROWS_FUSED = max(128, min(8192, int(os.environ.get("EXL3_FUSED_TEMP_ROWS", "128") or 128)))
MOE_ACT_SILU = 0
...
def make_linear_exl3(
    ...
    *,
    ...
    `marker` is the artifact's codebook multiplier tensor; it is handed to
    LinearEXL3 as `mcg=` or `mul1=`, which is what selects the trellis decode
    math (`decode_3inst<1>` vs `decode_3inst<2>` in exllamav3_ext).
    """
    if codebook not in SUPPORTED_CODEBOOKS:
        raise ValueError(f"unsupported EXL3 codebook={codebook!r}")
    cls = load_linear_exl3_cls()
    kwargs = {codebook: marker.contiguous()}
    return cls(
        config=None,
...
def apply_exl3_fused_moe(
    ...
    codebook = str(getattr(layer, "_exl3_codebook", "mcg"))
    # exl3_moe takes (gate_mcg, gate_mul1, up_mcg, up_mul1, down_mcg, down_mul1);
    # exactly one of each pair selects the trellis decode math.
    is_mcg = codebook == "mcg"
    is_mul1 = codebook == "mul1"
    if not (is_mcg or is_mul1):
        raise RuntimeError(f"EXL3 fused MoE: unsupported codebook={codebook!r}")
    args = (
        xh,
        out,
        expert_count,
...
_LEDGER_EXPERT = re.compile(
    r"layers\.(?P<layer>\d+)\.mlp\.experts\.\d+\.(?:gate|up|down)_proj$"
)
_LAYER_IN_PREFIX = re.compile(r"layers\.(?P<layer>\d+)\b")


def _layer_bits_from_ledger(ledger: Any) -> dict[int, int]:
    """Per-layer routed-expert rate from the checkpoint's `tensor_storage`.

    A uniform artifact yields one rate for every layer; a mosaic yields the
    upgraded layers at their own rate. Conflicting rates inside one layer are
    a packing error and fail closed rather than silently allocating one of them.
    """
    if not isinstance(ledger, dict):
        return {}
    bits: dict[int, int] = {}
    for key, entry in ledger.items():
        match = _LEDGER_EXPERT.search(key)
        if match is None or not isinstance(entry, dict):
            continue
        value = entry.get("bits_per_weight")
        if value is None:
            continue
        layer = int(match.group("layer"))
        value = int(value)
        previous = bits.setdefault(layer, value)
        if previous != value:
            raise ValueError(
                "EXL3 ledger declares two expert rates for layer "
                f"{layer}: {previous} and {value}"
            )
    return bits
...
class Exl3Config(QuantizationConfig):
    """Routed-experts-only EXL3 trellis (codebook `mcg` or `mul1`).

    Dense / shared / attention stay native. Mixed-precision artifacts (a
    "mosaic": some MoE layers re-quantized at a higher rate) declare their
    per-layer rate in the checkpoint's `tensor_storage` ledger; `layer_bits`
    carries it so each layer allocates its own trellis width.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__()
        # `bits` may be an average (a mosaic ships e.g. 2.05); the per-layer
        # ledger is authoritative for allocation and the average only feeds
        # the fallback for layers the ledger does not name.
        self.declared_bits = float(bits)
        self.bits = int(self.declared_bits)
        ...
        if self.codebook not in SUPPORTED_CODEBOOKS:
            raise ValueError(
                "this overlay implements codebook="
                f"{'/'.join(SUPPORTED_CODEBOOKS)}; got {self.codebook!r}"
            )
        allowed = (2, 3, 4, 5, 6)
        if self.bits not in allowed:
            raise ValueError(f"unsupported EXL3 bits={self.bits}")
        bad = sorted({b for b in self.layer_bits.values() if b not in allowed})
        if bad:
            raise ValueError(f"unsupported EXL3 per-layer bits={bad}")
    ...
    def bits_for_layer(self, layer_index: int | None) -> int:
        if layer_index is None:
            return self.bits
        return self.layer_bits.get(int(layer_index), self.bits)

    def marker_value(self) -> int:
        return CODEBOOK_MARKER[self.codebook]

    def suffixes(self) -> tuple[str, ...]:
        return EXL3_BASE_SUFFIXES + (self.codebook,)
    ...
    def from_config(cls, config: dict[str, Any]) -> "Exl3Config":
        skip = {
            ...
            "rank_stacked_tp",
            "quant_method",
            # tr3 ships a 37 MiB per-tensor ledger, a mosaic a 46 MiB one;
            # keep the raw ledger off the config object, retain only the
            # per-layer expert rate distilled from it.
            "tensor_storage",
        }
        return cls(
...
class Exl3MoEMethod(FusedMoEMethodBase):
    """Packed EXL3 trellis experts: create/load packed tensors, LinearEXL3 apply.

    One instance per MoE layer, so `bits` is the rate of *this* layer — a
    mosaic mixes 2- and 3-bit layers in one checkpoint.
    """
        ...
        **extra_weight_attrs,
        ...
        def new_packed() -> dict[str, Parameter]:
            return {
                ...
                f"w13_{self.codebook}": Parameter(
        ...
        if rank_stacked_tp > 1:
            ...
            for archive_rank in range(rank_stacked_tp):
                ...
                w2 = torch.nn.Module()
                packed = new_packed()
                for suffix in self.quant_config.suffixes():
    ...
    def _load_exl3(
        ...
        # need materialization; the scalar codebook marker costs four bytes.
        is_marker = suffix in SUPPORTED_CODEBOOKS
        if is_marker and suffix != self.codebook:
            raise RuntimeError(
                f"EXL3 checkpoint carries a {suffix!r} marker but the config "
                f"declares codebook={self.codebook!r}: {weight_name}"
            )
        loaded = loaded_weight
        if get_file_tensor_source(loaded) is not None:
            if self.quant_config.rank_stacked_tp is None or (
                is_marker and loaded.ndim == 0
            ):
                loaded = materialize_weight(loaded)
        else:
            loaded = loaded.detach().contiguous()
        # Safetensors preserves the artifact's scalar marker as shape (), while
        # RoutedExperts stores one marker per projection as shape (1,).
        # They are the same single int32 value; normalize only that exact ABI.
        if is_marker and loaded.ndim == 0:
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        # Pointer construction and marker validation consume loaded values.
            ...
            for archive_rank in range(rank_stacked_tp):
                ...
                for module in (w13, w2):
                    for param in module.parameters():
                        param._exl3_owner = layer
                # Rank submodules register the marker under the codebook name;
                # expose it as `.marker` so the stitching below is uniform.
                packed_ranks.append(
                    (
                        types.SimpleNamespace(
                            ...
                            suh=w13.suh,
                            svh=w13.svh,
                            marker=getattr(w13, self.codebook),
                        ),
                        types.SimpleNamespace(
                            trellis=w2.trellis,
                            suh=w2.suh,
                            svh=w2.svh,
                            marker=getattr(w2, self.codebook),
                        ),
                    )
                )
        else:
            ...
            packed_ranks.append(
                (
                    types.SimpleNamespace(
                        ...
                        suh=layer.w13_suh,
                        svh=layer.w13_svh,
                        marker=getattr(layer, f"w13_{self.codebook}"),
                    ),
                    types.SimpleNamespace(
                        trellis=layer.w2_trellis,
                        suh=layer.w2_suh,
                        svh=layer.w2_svh,
                        marker=getattr(layer, f"w2_{self.codebook}"),
                    ),
                )
            )

        expected_marker = self.quant_config.marker_value()
        marker13 = torch.cat([w13.marker.reshape(-1) for w13, _ in packed_ranks])
        marker2 = torch.cat([w2.marker.reshape(-1) for _, w2 in packed_ranks])
        if not torch.all(marker13 == expected_marker) or not torch.all(
            marker2 == expected_marker
        ):
            raise RuntimeError(
                f"EXL3 {self.codebook} marker is not the expected int32 "
                f"{expected_marker}; packed ABI mismatch"
            )
        ...
        if not self._logged:
            if fused_ok:
                logger.info(
                    "EXL3 %s trellis engaged for routed experts: layer=%s bits=%s "
